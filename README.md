...
### Role preview — "ดูในมุมมองของ…" (OBRS-1721)

Both shells' profile menus offer a **read-only role preview** to a user who *holds* `admin` or `owner` (`AuthService.getPreviewableRoles()`, keyed on `getHeldRoles()` so a salesperson never sees it). Admin may preview owner/salesperson/driver, owner may preview salesperson/driver; `customer` is never offered, because `auth.guard.ts`'s `customerArea` branch runs no role check and the preview would change nothing. The override lives at the single choke point `AuthService.getRoles()` — the guard, `hasAnyRole`, `hasHeldRole`, both nav builders and every `getRoles().includes(...)` call site narrow automatically, so **never special-case a page for the preview**; a page that needs it is a page bypassing the choke point. `app-role-preview-banner` (rendered from `app.component.html`, above the outlet, like `app-booking-closed-notice`) names the previewed role and states that the data is still the viewer's own — true, because the backend scopes by identity, not by role. Writes are blocked at `preview-readonly.interceptor.ts` (every `POST/PUT/PATCH/DELETE` to `/api/`, `/api/auth/` exempt), not by disabling buttons, because the real role already passes every previewed role's `@PreAuthorize`. State is in memory only — a refresh or a new tab returns to the real role — and exiting deliberately does not redirect. Adding a shell nav item or a role-gated menu entry needs nothing extra; adding a new nav *builder* must be re-run from `rebuildNav()`. See `docs/adr/0042-view-as-role-preview.md`.
