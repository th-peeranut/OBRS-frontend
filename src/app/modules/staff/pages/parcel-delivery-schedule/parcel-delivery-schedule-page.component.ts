...
interface ParcelDeliveryScheduleRow {
  id: number;
  tripId: string;
  departure: string;
  route: string;
...
/**
 * `/staff/parcels/deliveries` — schedule picker for the delivery-handoff
 * flow. Deliberately mirrors `BoardingEntryPageComponent` (same
 * driver/salesperson schedule-source split, same row shape) rather than
 * forking a new schedule-list query — the only difference is the navigation
 * target (`/staff/parcels/deliveries/:scheduleId` instead of
 * `/staff/boarding/:scheduleId`).
 */
@Component({
  selector: 'app-parcel-delivery-schedule-page',
  templateUrl: './parcel-delivery-schedule-page.component.html',
  styleUrl: './parcel-delivery-schedule-page.component.scss',
})
export class ParcelDeliveryEntryPageComponent implements OnInit, OnDestroy {
  protected rows: ParcelDeliveryScheduleRow[] = [];
  ...
  protected viewDeliveries(row: ParcelDeliveryScheduleRow): void {
    void this.router.navigate(['/staff/parcels/deliveries', row.id]);
  }
