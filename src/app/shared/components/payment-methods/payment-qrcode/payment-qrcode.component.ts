...
export class PaymentQrcodeComponent implements OnInit, OnChanges, OnDestroy {
  ...
  /** OBRS-1985: true when `paymentIdempotencyKey` came back from storage rather than being
   *  generated by this call — a create-payment retry made with that key failing is what
  ...
  private async ensurePromptPayQrCode(showMissingBookingAlert = false): Promise<void> {
    ...
    // OBRS-1985: in-memory key (this same instance already requested one this mount) →
    // stored key for THIS booking (a previous mount got one and the tab was re-entered/
    // refreshed) → only then a newly generated one. A newly generated key is written to
    // storage immediately, so the very next mount can reuse it instead of minting its own.
    if (!this.paymentIdempotencyKey) {
      const storedKey = readActiveBookingPaymentIdempotencyKey(bookingId);
      if (storedKey) {
        this.paymentIdempotencyKey = storedKey;
        this.isPaymentIdempotencyKeyRestored = true;
      } else {
        this.paymentIdempotencyKey = generateIdempotencyKey();
        writeActiveBookingPaymentIdempotencyKey(bookingId, this.paymentIdempotencyKey);
      }
    }
    const idempotencyKey = this.paymentIdempotencyKey;
    ...
    try {
      ...
      this.hasRequestedQrCode = false;
      // OBRS-1985: a create-payment call made with a RESTORED key that still fails proves
      // that key is dead (the backend's 24h idempotency record aged out, or the underlying
      // charge is already dead at the gateway) — so it must not be replayed on every future
      // mount. A freshly generated key failing here is an ordinary payment failure, not
      // evidence the key is bad, so it is left in storage for a retry to reuse.
      if (this.isPaymentIdempotencyKeyRestored) {
        clearActiveBookingPaymentIdempotencyKey();
        this.paymentIdempotencyKey = '';
        this.isPaymentIdempotencyKeyRestored = false;
      }
  ...
  private completePayment(): void {
    ...
    this.paymentIdempotencyKey = '';
    this.isPaymentIdempotencyKeyRestored = false;
    // OBRS-1985: the key belongs to the payment it was generated for, so it dies with it —
    // same reasoning as the in-memory reset above.
    clearActiveBookingPaymentIdempotencyKey();
    this.isWaitingForConfirmation = false;
