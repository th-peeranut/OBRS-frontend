...
@Component({
    selector: 'app-report-usability-modal',
    templateUrl: './report-usability-modal.component.html',
    styleUrl: './report-usability-modal.component.scss',
    standalone: false
})
export class ReportUsabilityModalComponent implements OnInit, OnDestroy {
  ...
  private readonly optionalEmail = (control: AbstractControl): ValidationErrors | null => {
    ...
    return ReportUsabilityModalComponent.EMAIL_PATTERN.test(val) ? null : { email: true };
  };
  ...
  constructor(
    ...
    private readonly modalService: ReportUsabilityModalService
  ) {}
  ...
  ngOnInit(): void {
    ...
    // OBRS-1832: every `<app-report-trigger>` in every shell opens THIS one
    // instance. The triggers own a position; the modal owns the form.
    this.modalService.openRequests$
      .pipe(takeUntil(this.destroy$))
