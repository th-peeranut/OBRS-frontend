...
describe('delayDisclosureOf (OBRS-1141)', () => {
  it('returns null when the backend sent no planned time — the round is not delayed (AC2)', () => {
    expect(delayDisclosureOf('2026-08-08T07:00:00+07:00', null)).toBeNull();
    expect(delayDisclosureOf('2026-08-08T07:00:00+07:00', undefined)).toBeNull();
    expect(delayDisclosureOf('2026-08-08T07:00:00+07:00', '')).toBeNull();
  });
  ...
  it('returns null for a zero-length delay rather than rendering "07:00 -> 07:00 (delayed)"', () => {
    ...
    expect(
      delayDisclosureOf('2026-08-08T07:00:00+07:00', '2026-08-08T07:00:00+07:00')
    ).toBeNull();
  });

  it('returns null when the planned time is AFTER the effective one', () => {
    expect(
      delayDisclosureOf('2026-08-08T07:00:00+07:00', '2026-08-08T09:00:00+07:00')
    ).toBeNull();
  });

  it('returns null for an unparseable timestamp instead of printing "Invalid Date"', () => {
    expect(delayDisclosureOf('2026-08-08T09:00:00+07:00', 'not-a-date')).toBeNull();
    expect(delayDisclosureOf('not-a-date', '2026-08-08T07:00:00+07:00')).toBeNull();
  });

  it('reports whole minutes for a sub-hour delay', () => {
    const disclosure = delayDisclosureOf(
      '2026-08-08T07:45:00+07:00',
      '2026-08-08T07:00:00+07:00'
    );
